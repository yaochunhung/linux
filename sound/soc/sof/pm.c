...
int snd_sof_resume(struct device *dev)
{
	...
	if (sdev->s0_suspend && sof_ops(sdev)->set_power_state) {
		/* resuming from D0I3 */
		...
		/* platform specific resuming from S0ix */
...
int snd_sof_suspend(struct device *dev)
{
	...
	if (sdev->s0_suspend && sof_ops(sdev)->set_power_state) {
		...
		if (ret < 0) {
			dev_warn(sdev->dev, "error: failed to enter D0I3, %d\n",
				 ret);
			return ret;
		}

		/* platform specific suspending to S0ix */
		return snd_sof_dsp_suspend(sdev);
	}

	/* else, run regular D3 suspend */
