...
static bool cnl_pm_gate_compress(struct snd_sof_ipc_msg *msg,
				 u32 *dr, u32 *dd)
{
...
static const struct hda_ipc compact_ipc[] = {
	{.header = SOF_IPC_GLB_PM_MSG | SOF_IPC_PM_GATE,
	 .compress = cnl_pm_gate_compress},
};
...
static bool cnl_is_ipc_compact(struct snd_sof_ipc_msg *msg)
{
	...
	for (i = 0; i < ARRAY_SIZE(compact_ipc); i++)
		if (msg->header == compact_ipc[i].header)
			return true;
				     ...
				     u32 *dr, u32 *dd)
{
	...
	for (i = 0; i < ARRAY_SIZE(compact_ipc); i++)
		if (msg->header == compact_ipc[i].header)
			return compact_ipc[i].compress(msg, dr, dd);
			    ...
			    struct snd_sof_ipc_msg *msg)
{
	...
	if (cnl_is_ipc_compact(msg)) {
		/* first, configure the IPC registers */
		if (!cnl_compact_ipc_compress(msg, &dr, &dd))
			return -EINVAL;

		/* then, send the message via IPC registers */
