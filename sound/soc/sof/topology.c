			     ...
			     struct sof_ipc_dai_config *config)
{
	...
	/* init IPC */
	memset(&config->afe, 0, sizeof(struct sof_ipc_dai_mtk_afe_params));
	config->hdr.size = size;
	...
	if (!dai) {
		dev_err(scomp->dev, "failed to find dai %s in %s",
